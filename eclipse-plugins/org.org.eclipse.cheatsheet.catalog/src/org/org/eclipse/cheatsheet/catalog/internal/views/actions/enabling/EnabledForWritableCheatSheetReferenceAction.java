...
public abstract class EnabledForWritableCheatSheetReferenceAction extends ResolvedAction<CheatSheetCatalogView> {

	public EnabledForWritableCheatSheetReferenceAction(CheatSheetCatalogView actionHost) {
		super(actionHost);
	}
	...
	public boolean isEnabled() {
		...
		try {
			if (item != null && item instanceof CheatSheetReference && !((CheatSheetReference) item).getParent().getParent().getReadOnly()) {
				result = true;
			}
		} catch (Exception e) {
			result = false;
