...
/**
 * The Class RefreshGroupJob.
 */
public class RefreshGroupJob extends Job {
	
	/** The group. */
	private final Group group;

	/** The Constant JOB_ID. */
	private static final String JOB_ID = "DWS: refreshing group ";

	/**
	 * Instantiates a new refresh group job.
	 * 
	 * @param group the group
	 */
	public RefreshGroupJob(Group group) {
		super(JOB_ID + group.getName());
		this.group = group;
	...
	protected IStatus run(IProgressMonitor monitor) {
		IStatus result = new StatusInfo(IStatus.OK, "Refresh of group: \"" + group.getUID() + "\" was successful.");
		try {
			monitor.beginTask("Refreshing group " + group.getUID(), 1000);
			...
			RepositoryModelPersistence.refreshGroup(group, externalInterruptionFlagSetter, modelItemListener);
			if (monitor.isCanceled()) {
				result = new StatusInfo(IStatus.WARNING, "Refresh of group: \"" + group.getUID() + "\" was canceled. It may not be complete.");
			}
		} catch (Exception e) {
			result = new StatusInfo(IStatus.ERROR, "Refresh of group: \"" + group.getUID() + "\"  failed: " + e.getMessage());
		} finally {
			monitor.done();
		}
		return result;
