 ...
 */
public class DownloadToLocalFileSystemJob extends Job {

	/** The artifact versions. */
	private final Set<ArtifactVersion> artifactVersions;

	/** The model item. */
	@SuppressWarnings("unchecked")
	private final IModelItem modelItem;

	private final String localPath;

	/** The Constant JOB_ID. */
	private final static String JOB_ID = "DWS: Downloading to local file system ";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public DownloadToLocalFileSystemJob(IModelItem modelItem, Set<ArtifactVersion> artifactVersions, String localPath) {
		super(JOB_ID);
	...
	private void download(Set<ArtifactVersion> artifactVersions, final IProgressMonitor monitor) {
		...
		for (ArtifactVersion artifactVersion : artifactVersions) {
			if (!monitor.isCanceled()) {
				...
				File toFile = new Path(localPath).toFile();
				if (toFile != null) {
					try {
						monitor.subTask("Downloading " + artifactVersion.getUID() + " to " + localPath);
					...
					throw new ProjectInteractionException("the target file :[" + localPath + "] could not be created.");
				}
			}
		}
