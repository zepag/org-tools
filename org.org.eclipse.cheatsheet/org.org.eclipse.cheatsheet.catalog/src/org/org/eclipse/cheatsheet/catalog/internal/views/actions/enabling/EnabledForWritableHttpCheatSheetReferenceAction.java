...
public abstract class EnabledForWritableHttpCheatSheetReferenceAction extends ResolvedAction<CheatSheetCatalogView> {

	public EnabledForWritableHttpCheatSheetReferenceAction(CheatSheetCatalogView actionHost) {
		super(actionHost);
	}
	...
	public boolean isEnabled() {
		...
		try {
			if (item != null && item instanceof CheatSheetReference && ((CheatSheetReference) item).getType() == CheatSheetReferenceType.HTTP && !((CheatSheetReference) item).getParent().getParent().getReadOnly()) {
				result = true;
			}
		} catch (Exception e) {
			result = false;
