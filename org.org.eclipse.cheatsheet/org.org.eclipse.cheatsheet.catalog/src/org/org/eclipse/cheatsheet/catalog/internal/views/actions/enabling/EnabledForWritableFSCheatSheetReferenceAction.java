...
public abstract class EnabledForWritableFSCheatSheetReferenceAction extends ResolvedAction<CheatSheetCatalogView> {

	public EnabledForWritableFSCheatSheetReferenceAction(CheatSheetCatalogView actionHost) {
		super(actionHost);
	}
	...
	public boolean isEnabled() {
		...
		try {
			if (item != null && item instanceof CheatSheetReference && ((CheatSheetReference) item).getType()==CheatSheetReferenceType.FILE_SYSTEM && !((CheatSheetReference) item).getParent().getParent().getReadOnly()) {
				result = true;
			}
		} catch (Exception e) {
			result = false;
