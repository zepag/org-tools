...
/**
 * The Class RefreshRepositoryJob.
 */
public class RefreshRepositoryJob extends Job {
	...
	/** The Constant JOB_ID. */
	private static final String JOB_ID = "DWS: refreshing crawledRepository ";

	/**
	 * Instantiates a new refresh crawledRepository job.
	 * 
	 * @param crawledRepository the crawledRepository
	 */
	public RefreshRepositoryJob(CrawledRepository crawledRepository) {
	...
	protected IStatus run(IProgressMonitor monitor) {
		IStatus result = new StatusInfo(IStatus.OK, "Refresh of crawledRepository: \"" + crawledRepository.getUID() + "\" was successful.");
		try {
			monitor.beginTask("Refreshing crawledRepository " + crawledRepository.getUID(), 1000);
			...
			IModelItemListener modelItemListener = new IModelItemListener() {
				public void changeOccured(ModelItemEvent modelItemEvent) {
					...
					if (modelItemEvent.getEventType() == ModelItemEvent.EventType.PRE_UPDATE_CHILD) {
						if (modelItemEvent.getTargetItem() != null && modelItemEvent.getSourceItem() != null) {
							fMonitor.subTask("Updating " + modelItemEvent.getTargetItem().getUID() +" in " +modelItemEvent.getSourceItem().getUID()+".");
							fMonitor.worked(1);
						}
						DWSCorePlugin.getDefault().notifyRepositoryModelUpdate(modelItemEvent.getTargetItem());
					}
			...
			RepositoryModelPersistence.refreshRepository(crawledRepository, externalInterruptionFlagSetter,modelItemListener);
			if (monitor.isCanceled()) {
				result = new StatusInfo(IStatus.WARNING, "Refresh of crawledRepository: \"" + crawledRepository.getUID() + "\" was canceled. It may not be complete.");
			}
		} catch (Exception e) {
			result = new StatusInfo(IStatus.ERROR, "Refresh of crawledRepository: \"" + crawledRepository.getUID() + "\"  failed: " + e.getMessage());
		} finally {
			monitor.done();
		}
		return result;
