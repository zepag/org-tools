...
import org.org.eclipse.dws.ui.internal.handlers.OpenDetailsViewHandler;


/**
 * The Class OpenDetailsViewAction.
 */
public class OpenDetailsViewAction extends AbstractWorkbenchWindowAction {
	
	/** The Constant ACTION_LABEL_ID. */
	public static final String ACTION_LABEL_ID = ActionsMessages.OpenDetailsViewAction_id;
	...
	public void run(IAction action) {
		run(new Runnable() {
			public void run() {
				...
				OpenDetailsViewHandler openDetailsViewHandler = new OpenDetailsViewHandler();
				try {
					openDetailsViewHandler.execute(new ExecutionEvent(null, Collections.EMPTY_MAP, null, handlerService.getCurrentState()));
				} catch (ExecutionException e) {
					ErrorDialog errorDialog = new ErrorDialog(ACTION_LABEL_ID, "Problem while trying to open details view", e);
					errorDialog.open();
				}
			}
