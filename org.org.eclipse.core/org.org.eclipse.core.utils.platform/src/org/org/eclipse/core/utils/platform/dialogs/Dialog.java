...
public abstract class Dialog extends Window {
	protected static final int COMPOSITE_STYLE=SWT.NONE; 
	/**
	...
	protected Dialog(Shell parentShell) {
		...
		if (parentShell == null && Policy.DEBUG_DIALOG_NO_PARENT)
			Policy.getLog().log(new Status(IStatus.INFO, Policy.JFACE, IStatus.INFO, this.getClass() + " created with no shell",//$NON-NLS-1$
					new Exception()));
	}
	...
	protected Control createContents(Composite parent) {
		...
		// composite.setBackground(JFaceColors.getBannerBackground(form.getBody().getDisplay()));
