...
public class RepositoryCrawlerService<T extends ICrawledRepositorySetup> {
	public final static Pattern ARTIFACT_VERSION_FOLDER_PATTERN = Pattern.compile("[0-9.]*[0-9].+");

	public final static Pattern JAVADOC_ARTIFACT_PATTERN = Pattern.compile(".*-javadoc.jar");

	public final static Pattern SOURCES_ARTIFACT_PATTERN = Pattern.compile(".*-sources.jar");
	 ...
	 * 
	 */
	public RepositoryCrawlerService(IRepositoryBrowserPlugin<T> repositoryBrowserPlugin) {
	...
	public void addExternalInterruptionFlagSetter(int index, IExternalInterruptionFlagSetter externalInterruptionFlagSetter) {
		externalInterruptionFlagSetters.add(index, externalInterruptionFlagSetter);
	}

	public Queue<Entry> scanForArtifacts(T repositorySetup, String upperGroupName, String folderName) throws IOException {
		String requestedUrl = repositoryBrowserPlugin.buildUrl(repositorySetup, makeGroupPathOutOfGroupName(upperGroupName), folderName);
		Queue<Entry> artifactsList = new LinkedBlockingQueue<Entry>();
		Queue<Entry> entryList = repositoryBrowserPlugin.getEntryList(requestedUrl);
	...
	@SuppressWarnings("unchecked")
	private <Y extends IModelItem> void scanRepository(ScanningContext<Y, T> scanningContext) throws IOException {
		processFlagSetters();
		if (getInterruptionFlag().isCurrentStatus(InterruptionFlag.CONTINUE)) {
			// create the requested URL
			String requestedUrl = repositoryBrowserPlugin.buildUrl(scanningContext.getRepositorySetup(), scanningContext.getUpperGroupName(), "");
			// get the entries from the listing page
			Queue<Entry> entryList = repositoryBrowserPlugin.getEntryList(requestedUrl);
			// test if the current folder is an artifact folder
			boolean isArtifact = RepositoryCrawlerHelper.isCurrentFolderAnArtifactFolder(entryList);
			// keeps the group pathes only (folders which
			// are part of a group id), removes version folders.
			Queue<Entry> groupPathFolders = RepositoryCrawlerHelper.keepGroupPathOnly(entryList);
			// if the current folder is recognized as an artifact folder,
			// retrieve the artifacts' info and add it to the upper Group.
			if (isArtifact) {
				scanForArtifact(scanningContext, RepositoryCrawlerHelper.getVersionsFolders(entryList), groupPathFolders, requestedUrl);
			}
			// handle recursively the other group folders.
			if (worthLookingFurtherInTree(scanningContext)) {
				handleGroupPathFolders(scanningContext, scanningContext.getUpperGroupName(), groupPathFolders);
			}
		}
		try {
			while (getInterruptionFlag().isCurrentStatus(InterruptionFlag.PAUSE)) {
				Thread.sleep(1000);
			}
	...
	private Queue<Entry> removeNonMatchingFolders(T repositorySetup, String upperGroupName, Queue<Entry> groupPathFolders) {
		...
		for (Entry entry : groupPathFolders) {
			...
			for (String filter : repositorySetup.getGroupFilters()) {
				...
				final String separator = (upperGroupName.equals("") || upperGroupName.endsWith(".")) ? "" : "/";
				final String toTest = upperGroupName + separator + entry.getResolvedName();
				if (toTest.matches(filter) || filter.startsWith(toTest) || toTest.startsWith(filter)) {
					matchesAny = true;
					break;
				}
			}
	...
	private boolean worthLookingFurtherInTree(ScanningContext scanningContext) {
		...
		for (String groupFilter : scanningContext.getRepositorySetup().getGroupFilters()) {
			if (groupFilter != null) {
				...
				if (upperGroupName.matches(groupFilter) || (groupFilter.startsWith(upperGroupName))) {
					result = true;
					break;
				}
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private <Y extends IModelItem> void handleGroupPathFolders(ScanningContext<Y, T> scanningContext, String folderName, Queue<Entry> groupPathFolders) throws IOException {
		String upperGroupName = scanningContext.getUpperGroupName();
		groupPathFolders = removeNonMatchingFolders(scanningContext.getRepositorySetup(), upperGroupName, groupPathFolders);
		for (Entry groupFolderEntry : groupPathFolders) {
			String groupName = "";
			String cleanFolderName = repositoryBrowserPlugin.cleanFolderName(groupFolderEntry.getResolvedName());
	 ...
	 */
	@SuppressWarnings("unchecked")
	private <Y extends IModelItem> void scanForArtifact(ScanningContext<Y, T> scanningContext, Queue<Entry> versionsList, Queue<Entry> groupPathFolders, String requestedUrl) throws IOException {
		if (!RepositoryCrawlerHelper.isRootFolder(scanningContext.getUpperGroupName())) {
			if (isWorthScanningForArtifact(scanningContext)) {
				groupPathFolders.removeAll(versionsList);
				Set<ArtifactVersion> artifactVersions = generateArtifactVersions(scanningContext.getRepositorySetup(), scanningContext.getUpperGroupName(), requestedUrl, versionsList);
	...
	@SuppressWarnings("unchecked")
	private <Y extends IModelItem> Boolean isWorthScanningForArtifact(ScanningContext<Y, T> scanningContext) {
		Boolean result = true;// default is that it is worth it.
		if (scanningContext.getRepositorySetup().hasGroupFilters()) {
			result = false;
			String group = scanningContext.getUpperGroupName().substring(0, scanningContext.getUpperGroupName().lastIndexOf("/"));
			for (String groupFilter : scanningContext.getRepositorySetup().getGroupFilters()) {
				groupFilter = makeGroupPathOutOfGroupName(groupFilter);
				// if a strict group name is specified
	...
	@SuppressWarnings("unchecked")
	private <Y extends IModelItem> void addToResult(ScanningContext<Y, T> scanningContext, Set<ArtifactVersion> artifactVersions) {
		if (scanningContext.getResultingItem() instanceof CrawledRepository) {
			addToRepository((ScanningContext<CrawledRepository, T>) scanningContext, artifactVersions);
		} else if (scanningContext.getResultingItem() instanceof Group) {
			addToGroup((ScanningContext<Group, T>) scanningContext, artifactVersions);
		} else if (scanningContext.getResultingItem() instanceof Artifact) {
			addToArtifact((ScanningContext<Artifact, T>) scanningContext, artifactVersions);
		}
	}

	private void addToRepository(ScanningContext<CrawledRepository, T> scanningContext, Set<ArtifactVersion> artifactVersions) {
		Group group = null;
		String groupName = makeGroupNameOutOfGroupPath(RepositoryCrawlerHelper.popNameSegment(scanningContext.getUpperGroupName()));
		if (scanningContext.getResultingItem().hasChild(groupName)) {
			...
			if (!scanningContext.hasTemporaryGroup(groupName)) {
				group = new Group(makeGroupNameOutOfGroupPath(RepositoryCrawlerHelper.popNameSegment(scanningContext.getUpperGroupName())));
				scanningContext.addTemporaryGroup(group);
			} else {
				group = scanningContext.getTemporaryGroup(makeGroupNameOutOfGroupPath(groupName));
			}
			scanningContext.getResultingItem().addChild(group);
		}
		Artifact artifact = new Artifact(RepositoryCrawlerHelper.getLastGroupNameSegment(scanningContext.getUpperGroupName()));
	...
	private void addToGroup(ScanningContext<Group, T> scanningContext, Set<ArtifactVersion> artifactVersions) {
		Artifact artifact = new Artifact(RepositoryCrawlerHelper.getLastGroupNameSegment(scanningContext.getUpperGroupName()));
		if (!scanningContext.getResultingItem().hasChild(artifact.getUID())) {
			scanningContext.getResultingItem().addChild(artifact);
		}
		for (ArtifactVersion artifactVersion : artifactVersions) {
			scanningContext.getResultingItem().getChild(artifact.getUID()).addChild(artifactVersion);
		}
	}

	private void addToArtifact(ScanningContext<Artifact, T> scanningContext, Set<ArtifactVersion> artifactVersions) {
		for (ArtifactVersion artifactVersion : artifactVersions) {
	 ...
	 */
	private Set<ArtifactVersion> generateArtifactVersions(T repositorySetup, String upperGroupName, String requestedUrl, Queue<Entry> versionsList) throws IOException, MalformedURLException {
		...
		for (Entry versionFolder : versionsList) {
			String version = repositoryBrowserPlugin.cleanFolderName(versionFolder.getResolvedName());
			...
			for (Entry artifactVersionEntry : scanForArtifacts(repositorySetup, upperGroupName, version)) {

				if (RepositoryCrawlerHelper.isSource(artifactVersionEntry)) {
					ArtifactVersion artifactVersion = new ArtifactVersion();
					artifactVersion.setId(artifactVersionEntry.getResolvedName());
					artifactVersion.setVersion(version);
					repositoryBrowserPlugin.setUrlForArtifactVersion(artifactVersion, requestedUrl, version);
					if (artifactVersionsMap.get(artifactVersion.getVersion()) != null) {
						artifactVersionsMap.get(artifactVersion.getVersion()).setSourcesUrl(artifactVersion.getUrl());
					} else {
						sourcesStack.put(artifactVersion.getVersion(), artifactVersion);
					}
				} else if (RepositoryCrawlerHelper.isJavadoc(artifactVersionEntry)) {
					ArtifactVersion artifactVersion = new ArtifactVersion();
					artifactVersion.setId(artifactVersionEntry.getResolvedName());
					artifactVersion.setVersion(version);
					repositoryBrowserPlugin.setUrlForArtifactVersion(artifactVersion, requestedUrl, version);
					if (artifactVersionsMap.get(artifactVersion.getVersion()) != null) {
						artifactVersionsMap.get(artifactVersion.getVersion()).setJavadocUrl(artifactVersion.getUrl());
					} else {
						javadocStack.put(artifactVersion.getVersion(), artifactVersion);
					}
				} else if (RepositoryCrawlerHelper.isArchive(artifactVersionEntry, archivesSpecification) || RepositoryCrawlerHelper.isLibrary(artifactVersionEntry, librariesSpecification)) {
					ArtifactVersion artifactVersion = new ArtifactVersion();
					artifactVersion.setId(artifactVersionEntry.getResolvedName());
					artifactVersion.setVersion(version);
					final String classifier = RepositoryCrawlerHelper.getClassifier(artifactVersionEntry, version);
					if (classifier != null && !version.contains(classifier)) {
						artifactVersion.setClassifier(classifier);
					}
					if (RepositoryCrawlerHelper.isLibrary(artifactVersionEntry, librariesSpecification)) {
						artifactVersion.setType(ArtifactVersion.Type.LIBRARY);
					} else if (RepositoryCrawlerHelper.isArchive(artifactVersionEntry, archivesSpecification)) {
						artifactVersion.setType(ArtifactVersion.Type.ARCHIVE);
					}
					repositoryBrowserPlugin.setUrlForArtifactVersion(artifactVersion, requestedUrl, version);
					final String artifactVersionIdentifier = artifactVersion.getVersion() + (artifactVersion.getClassifier() == null ? "" : "-" + artifactVersion.getClassifier());
					artifactVersionsMap.put(artifactVersionIdentifier, artifactVersion);
					if (pomStack.get(artifactVersionIdentifier) != null) {
						artifactVersion.setPomUrl(pomStack.get(artifactVersionIdentifier).getUrl());
						pomStack.remove(artifactVersionIdentifier);
					}
					if (javadocStack.get(artifactVersionIdentifier) != null) {
						artifactVersion.setJavadocUrl(javadocStack.get(artifactVersion.getVersion()).getUrl());
						javadocStack.remove(artifactVersion.getVersion());
					}
					if (sourcesStack.get(artifactVersionIdentifier) != null) {
						artifactVersion.setSourcesUrl(sourcesStack.get(artifactVersionIdentifier).getUrl());
						sourcesStack.remove(artifactVersionIdentifier);
					}
				} else if (RepositoryCrawlerHelper.isPom(artifactVersionEntry, pomSpecification)) {
					ArtifactVersion artifactVersion = new ArtifactVersion();
					artifactVersion.setId(artifactVersionEntry.getResolvedName());
					artifactVersion.setVersion(version);
					...
					if (artifactVersionsMap.get(artifactVersionIdentifier) != null) {
						artifactVersionsMap.get(artifactVersionIdentifier).setPomUrl(artifactVersion.getUrl());
					} else {
			...
			if (!pomStack.isEmpty()) {
				for (String pomVersion : pomStack.keySet()) {
					artifactVersionsMap.put(pomVersion + "pom", pomStack.get(pomVersion));
				}
			}
		}
		artifactVersions = new HashSet<ArtifactVersion>(artifactVersionsMap.values());
		return artifactVersions;
	}

	public CrawledRepository retrieveRepository(T repositorySetup, String label) {
		return this.retrieveRepository(repositorySetup, label, new IModelItemListener[0]);
	}

	@SuppressWarnings("unchecked")
	public CrawledRepository retrieveRepository(T repositorySetup, String label, IModelItemListener... modelItemListeners) {
		CrawledRepository crawledRepository = new CrawledRepository(label, repositorySetup);
		return refreshRepository(crawledRepository, label, modelItemListeners);
	}

	public CrawledRepository refreshRepository(CrawledRepository existingRepository, String label) {
		return this.refreshRepository(existingRepository, label, new IModelItemListener[0]);
	}

	@SuppressWarnings("unchecked")
	public CrawledRepository refreshRepository(CrawledRepository crawledRepository, String label, IModelItemListener... modelItemListeners) {
		// if a precise group definition is set, all existing groups should be added, in order not to lose them.
		if (crawledRepository.getRepositorySetup().hasGroupFilters() && crawledRepository.hasChildren()) {
			for (Group childGroup : crawledRepository.getChildren()) {
				boolean matchesNone = true;
				for (String filter : crawledRepository.getRepositorySetup().getGroupFilters()) {
					if (childGroup.getName().matches(filter)) {
						matchesNone = false;
					}
				}
				// if group doesn't match any of the specified filters, add it as a filter.
				if (matchesNone) {
		...
		ScanningContext<CrawledRepository, T> scanningContext = new ScanningContext<CrawledRepository, T>(crawledRepository, repositorySetup);
		try {
	...
	public Group retrieveGroup(T repositorySetup, String groupName) {
		return this.retrieveGroup(repositorySetup, groupName, new IModelItemListener[0]);
	}

	@SuppressWarnings("unchecked")
	public Group retrieveGroup(T repositorySetup, String groupName, IModelItemListener... modelItemListeners) {
		...
		setup.getGroupFilters().remove(AbstractCrawledRepositorySetup.KEEP_ALL_PATTERN);
		setup.getGroupFilters().add(groupName);
		return refreshGroup(group, (T) setup.getImmutable(), groupName, modelItemListeners);
	}

	public Group refreshGroup(Group group, T repositorySetup, String groupName) {
		return this.refreshGroup(group, repositorySetup, groupName, new IModelItemListener[0]);
	}

	@SuppressWarnings("unchecked")
	public Group refreshGroup(Group group, T repositorySetup, String groupName, IModelItemListener... modelItemListeners) {
		repositoryBrowserPlugin.checkRepositorySetup(repositorySetup);
		if (modelItemListeners != null) {
			for (IModelItemListener modelItemListener : modelItemListeners) {
		...
		ScanningContext<Group, T> context = new ScanningContext<Group, T>(group, repositorySetup);
	...
	public Artifact refreshArtifact(Artifact artifact, T repositorySetup, String groupName, String artifactName) {
		return this.refreshArtifact(artifact, repositorySetup, groupName, artifactName, new IModelItemListener[0]);
	}

	@SuppressWarnings("unchecked")
	public Artifact refreshArtifact(Artifact artifact, T repositorySetup, String groupName, String artifactName, IModelItemListener... modelItemListeners) {
		repositoryBrowserPlugin.checkRepositorySetup(repositorySetup);
		if (modelItemListeners != null) {
			for (IModelItemListener modelItemListener : modelItemListeners) {
		...
		ScanningContext<Artifact, T> context = new ScanningContext<Artifact, T>(artifact, repositorySetup);
		context.setUpperGroupName(makeGroupPathOutOfGroupName(groupName + "." + artifactName));
	...
	public Artifact retrieveArtifact(T repositorySetup, String groupName, String artifactName) {
		return this.retrieveArtifact(repositorySetup, groupName, artifactName, new IModelItemListener[0]);
	}

	@SuppressWarnings("unchecked")
	public Artifact retrieveArtifact(T repositorySetup, String groupName, String artifactName, IModelItemListener... modelItemListeners) {
		Artifact artifact = new Artifact(artifactName);
		AbstractCrawledRepositorySetup setup = repositorySetup.getMutable();
		setup.getGroupFilters().remove(AbstractCrawledRepositorySetup.KEEP_ALL_PATTERN);
		setup.getGroupFilters().add(groupName);
		return refreshArtifact(artifact, (T) setup, groupName, artifactName, modelItemListeners);
	}

	public Set<ArtifactVersion> retrieveArtifactVersions(T repositorySetup, String groupName, String artifactName, String artifactVersion) {
		Set<ArtifactVersion> result = new LinkedHashSet<ArtifactVersion>();
		repositoryBrowserPlugin.checkRepositorySetup(repositorySetup);
		repositoryBrowserPlugin.init(repositorySetup);

		// create the requested URL
		String requestedUrl = repositoryBrowserPlugin.buildUrl(repositorySetup, makeGroupPathOutOfGroupName(groupName), artifactName);
		// get the entries from the listing page
		try {
			Queue<Entry> entryList = repositoryBrowserPlugin.getEntryList(requestedUrl);
			...
			result = generateArtifactVersions(repositorySetup, groupName + "." + artifactName, requestedUrl, chosenVersionsList);
		} catch (IOException e) {
			throw new RepositoryCrawlingException(e);
		}
		return result;
	...
	public void setArchivesSpecification(ArchivesSpecification archivesSpecification) {
	...
	public void setLibrariesSpecification(LibrariesSpecification librariesSpecification) {
